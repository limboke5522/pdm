<script>
        ...
        if (!$.isEmptyObject(ObjData)) {
          $.each(ObjData.documentlist, function(key, value) {
            ...
            $.each(ObjData.usertype[value.ID], function(key_usertype, value_usertype) {
              ...
              StrTR_checkbox += "<td style='text-align: center;'><center>" + chkUser_Ad + "</center></td>";
            ...
            StrTR += "<tr style='border-radius: 15px 15px 15px 15px;margin-top: 6px;margin-bottom: 6px;'>" +
              ...
              "<td style='text-align: left;'>" + value.DocName + "</td>" + StrTR_checkbox
  ...
  function showData_Head() {
    $.ajax({
      url: "process/permission_doc.php",
      type: 'POST',
      data: {
        'FUNC_NAME': 'showData_Head'
      },
      success: function(result) {
        var ObjData = JSON.parse(result);
        var StrTR = "";
        if (!$.isEmptyObject(ObjData)) {
          StrTR += `<th class="bg_tableAll">เอกสาร</th>`;
          $.each(ObjData, function(key, value) {

            StrTR += `<th class="bg_tableAll"   style="width: 8%;">${value.UserType}</th>`;


          });
          $('#headtable').html(StrTR);
        }
      }
    });
  }
